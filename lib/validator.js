  ...
  validator.isEmailAddress = function(email) {
    ...
    if ( emailSplit.length === 2 ) {
      ...
      for ( var i = 0; i < addLen; i++ ) {
        ...
        if ( allowedChars.indexOf(address.charAt(i)) === -1 ) return false;
        // if the '.' is at the start or the end of the address
        if ( address.charAt(i) === '.' && ( i === 0 || i === addLen - 1 ) ) return false;
        // if there are two consecutive '.' characters
        if ( i > 0 && address.charAt(i - 1) === '.' && address.charAt(i) === '.' ) return false;
      }

      for ( var i = 0; i < domLen; i++ ) {
        // there are invalid characters or the underscore in the domain part
        if ( allowedChars.indexOf(domain.charAt(i)) === -1 || domain.charAt(i) === '_' ) return false;
        // if the '.' or '-' is at the start or the end of the domain
        if ( ( domain.charAt(i) === '.' || domain.charAt(i) === '-' ) && ( i === 0 || i === domLen - 1 ) ) return false;
        // if there are two consecutive '.' characters
        if ( i > 0 && domain.charAt(i - 1) === '.' && domain.charAt(i) === '.' ) return false;
      }
...
//
// checks if the input is the phone number, i.e. follows the appropriate phone number foramt
// in Serbia the format is: prefix(delimiter)number
// delimiter can be '/', '-', space or it may not exist
// the number can have 6 or 7 digits delimited after 3rd or 4th digith depending on the number of digits in the number
...
// '-' can delimit the prefix, the number can be delimited with another '-' but not with space, the number may also not be delimited
  ...
  validator.isPhoneNumber = function(input) {
    ...
    if (!input) throw 'Missing parameter in "isPhoneNumber" function: "number"';
...
// other characters than the english alphabet character to be valid
  ...
  validator.isCharacterSet = function(input, charSets) {
    ...
    for ( i = 0; i < inputLen; i++ ) {
      ...
      found = false;
      for ( key in charSets ) {
        if ( charSets[key].indexOf(char) !== -1 ) {
